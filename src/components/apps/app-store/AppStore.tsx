              ...
              <SidebarNav.Group title="Repos">
                ...
                {showRepoForm && (
                  <div className="mx-1 mt-1 flex flex-col gap-1 rounded-md bg-gray-alpha-100 p-2">
                    ...
                    />
                    <Button size="sm" onClick={addRepo} disabled={busy || !repoUrl.trim()}>
                      {actionStatus === "adding-repo" ? <Spinner size="sm" /> : "Add"}
                    </Button>
                  </div>
                )}
              </SidebarNav.Group>
                ...
                <div className="flex h-full flex-col">
                  ...
                  {total > PAGE_SIZE && (
                    <div className="flex shrink-0 items-center justify-between border-t border-gray-alpha-200 px-2 py-1">
                      <Button size="sm" variant="secondary" onClick={() => setOffset(Math.max(0, offset - PAGE_SIZE))} disabled={!hasPrev}>
                        <ChevronLeft />
                        Prev
                      </Button>
                      <span className="text-copy-13 text-gray-800">{pageNum} / {totalPages}</span>
                      <Button size="sm" variant="secondary" onClick={() => setOffset(offset + PAGE_SIZE)} disabled={!hasNext}>
                        Next
                        <ChevronRight />
                      </Button>
                    </div>
    ...
    <div className="flex h-full flex-col overflow-auto">
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {isInstalled && (
            <Button size="sm" onClick={() => onLaunch(pkg.name)}>
              <Play />
              Launch
            </Button>
          )}
          {!isInstalled && (
            <Button
              ...
              disabled={busy}
            >
              {actionStatus === "installing" ? <Spinner size="sm" /> : <Download />}
              Install{selectedVersion ? ` ${selectedVersion}` : ""}
            </Button>
          )}
          {isInstalled && (
            <Button size="sm" variant="destructive" onClick={() => onRemove(pkg.name)} disabled={busy}>
              {actionStatus === "removing" ? <Spinner size="sm" /> : <Trash />}
              Remove
            </Button>
          )}
        </div>
