    ...
    <div className="flex max-w-2xl flex-col gap-8 text-gray-1000">
            ...
            <div className="p-4">
              <div className="mb-4 flex items-center gap-3">
                <Tooltip>
                  ...
                  <TooltipContent side="top">Change icon</TooltipContent>
                </Tooltip>
              ...
              <div className="flex flex-wrap gap-2">
                {activeWorkspace.status === "active" && (
                  <>
                    <ActionButton size="sm" variant="outline" onClick={wrap("restart", () => restartWorkspace(activeWorkspace.id))} disabled={busy !== null} loading={busy === "restart"}>Restart</ActionButton>
                    <ActionButton size="sm" variant="secondary" onClick={wrap("snapshot", () => snapshotWorkspace(activeWorkspace.id))} disabled={busy !== null} loading={busy === "snapshot"}>Snapshot</ActionButton>
                    <ActionButton size="sm" variant="ghost" onClick={wrap("stop", () => stopWorkspace(activeWorkspace.id))} disabled={busy !== null} loading={busy === "stop"}>Stop</ActionButton>
                    <ActionButton size="sm" variant="destructive" onClick={wrap("kill", () => killWorkspace(activeWorkspace.id))} disabled={busy !== null} loading={busy === "kill"}>Kill</ActionButton>
                  </>
                )}
                {(activeWorkspace.status === "stopped" || activeWorkspace.status === "snapshotted") && (
                  <ActionButton size="sm" onClick={wrap("restart", () => restartWorkspace(activeWorkspace.id))} disabled={busy !== null} loading={busy === "restart"}>Start</ActionButton>
                )}
              </div>
            </div>
          ) : (
      ...
      <section>
        <SectionHeader title="All Workspaces">
          <ActionButton size="sm" onClick={wrap("new", () => createWorkspace())} disabled={busy !== null} loading={busy === "new"}>
            New Workspace
          </ActionButton>
        </SectionHeader>
            ...
            >
              ...
              <div className="ml-2 flex shrink-0 gap-1">
                {ws.status === "active" && ws.id !== activeWorkspaceId && (
                  <Button size="sm" variant="secondary" onClick={() => setActiveWorkspace(ws.id)}>Switch</Button>
                )}
                {ws.status === "active" && (
                  <ActionButton size="sm" variant="destructive" onClick={wrap(`kill-${ws.id}`, () => killWorkspace(ws.id))} disabled={busy !== null} loading={busy === `kill-${ws.id}`}>Kill</ActionButton>
                )}
                {(ws.status === "stopped" || ws.status === "snapshotted") && (
                  <ActionButton size="sm" onClick={wrap(`restart-${ws.id}`, () => restartWorkspace(ws.id))} disabled={busy !== null} loading={busy === `restart-${ws.id}`}>Start</ActionButton>
                )}
              </div>
            </div>
          ))}
