...
export async function GET(request: NextRequest) {
  ...
  if (!clientId) {
    return NextResponse.redirect(
      `${getBaseUrl()}/?error=${encodeURIComponent("VERCEL_CLIENT_ID is not configured")}`,
    );
  }
  ...
  // Try auth methods in this order:
  // 1) client_secret_basic (Authorization header)
  // 2) client_secret_post (form body)
  // 3) public client / no secret
  let tokenRes: Response | null = null;
  if (clientSecret) {
    const basic = Buffer.from(`${clientId}:${clientSecret}`).toString("base64");
    tokenRes = await fetch("https://api.vercel.com/login/oauth/token", {
      method: "POST",
      headers: {
    ...
    if (!tokenRes.ok) {
      const postBody = new URLSearchParams(baseBody);
  ...
  if (!tokenRes || !tokenRes.ok) {
    const errText = tokenRes ? await tokenRes.text() : "No token response";
    console.error("Token exchange failed:", errText);
    return NextResponse.redirect(
      `${getBaseUrl()}/?error=${encodeURIComponent("Token exchange failed")}`,
    );
  }
  ...
  if (!userInfoRes.ok) {
    ...
    return NextResponse.redirect(
      `${getBaseUrl()}/?error=${encodeURIComponent("Failed to fetch user info")}`,
    );
  }
