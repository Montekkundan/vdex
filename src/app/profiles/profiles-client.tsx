...
export function ProfilesClient() {
  ...
  const { policies, stats, limits: policyLimits, isLoading: policiesLoading } =
  ...
  const [policyProvider, setPolicyProvider] = useState("vercel");
  const [policyExperience, setPolicyExperience] = useState("gui");
  const [policyDisplayClient, setPolicyDisplayClient] = useState("xpra");
  ...
  function appendGuiTemplate(templateId: string) {
    ...
    if (!template) return;
    setBuildScript((prev) => (prev.trim() ? `${prev}\n${template.script}` : template.script));
  }
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[var(--container-max-width)] flex-col border-x border-dashed border-gray-alpha-300">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {snapshots.length}/{limits.maxSnapshotsPerUser} snapshots used.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              />
              <Select value={captureWorkspaceId} onValueChange={setCaptureWorkspaceId}>
                <SelectTrigger>
              ...
              >
                {busy === "capture" ? <Spinner className="size-3.5" /> : "Capture Snapshot"}
              </Button>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Select
                ...
                onValueChange={(value) => setBuildExperience(value as "gui" | "cli")}
              >
                <SelectTrigger>
              ...
              >
                {busy === "build" ? <Spinner className="size-3.5" /> : "Build Snapshot"}
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Use as launch source and warm-pool backing.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            {snapshots.map((snapshot) => (
              ...
              >
                <div className="min-w-0">
                  <p className="truncate text-copy-14 font-medium text-gray-1000">{snapshot.name}</p>
                  <p className="text-copy-12 text-gray-700">
                    {snapshot.provider} · {snapshot.experience} · {snapshot.displayClient ?? "none"} ·{" "}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      runWithState(`default-${snapshot.id}`, async () => {
                        await updateSnapshotMutate(snapshot.id, { isDefault: true });
                      })
                    }
                  >
                    Set default
            ...
            {snapshots.length === 0 ? (
              <p className="text-copy-13 text-gray-700">No snapshots yet.</p>
            ) : null}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-2 md:grid-cols-3">
              ...
              <Select value={policyExperience} onValueChange={setPolicyExperience}>
                <SelectTrigger>
                  <SelectValue placeholder="Experience" />
                </SelectTrigger>
                <SelectContent>
              ...
              <Select value={policyDisplayClient} onValueChange={setPolicyDisplayClient}>
                <SelectTrigger>
                  <SelectValue placeholder="Display client" />
                </SelectTrigger>
                <SelectContent>
                  {Object.values(DISPLAY_CLIENTS).map((c) => (
              ...
              <Select value={policySizeProfile} onValueChange={setPolicySizeProfile}>
                <SelectTrigger>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setPolicySourceType(value as "platform_default" | "user_snapshot")
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="user_snapshot">User snapshot</SelectItem>
                </SelectContent>
              </Select>
              <Input
              ...
              {policySourceType === "user_snapshot" ? (
                <Select value={policySnapshotRefId} onValueChange={setPolicySnapshotRefId}>
            ...
            >
              {busy === "policy-create" ? <Spinner className="size-3.5" /> : "Create policy"}
            </Button>
            ...
            <div className="space-y-2">
                ...
                >
                  <div>
                    <p className="text-copy-14 font-medium text-gray-1000">
                      {policy.provider} · {policy.experience} · {policy.displayClient} · {policy.sizeProfile}
                    </p>
                    <p className="text-copy-12 text-gray-700">
                      source={policy.snapshotRefType}
                      {policy.snapshotRefId ? ` (${policy.snapshotRefId.slice(0, 8)}...)` : ""}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">{policy.enabled ? "enabled" : "disabled"}</Badge>
                    <Input
                      className="w-20"
              ...
              {policies.length === 0 ? (
                <p className="text-copy-13 text-gray-700">No warm-pool policies yet.</p>
              ) : null}
            </div>
          </CardContent>
        </Card>
